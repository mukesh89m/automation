 ...
 */
public class EditWriteBoardResponseBeforeSubmittingGradableAssignment extends Driver{

    @Test(priority = 1, enabled = true)
    public void editWriteBoardResponseBeforeSubmittingGradableAssignment()
    {
        try{
            String appendChar = "a";
            int dataIndex = 113;
          ...
          //  new WriteBoard().enterTextInWriteBoard(str, dataIndex);//write in 10th question
            ...
            Assert.assertEquals(edited, true, "Instructor is unable to edit writeboard data for non gradable true false type question.");
            ...
            edited = new WriteBoard().verifyWriteBoardDataIsSaved(str1);
            System.out.println("edited: "+edited);
            Assert.assertEquals(edited, true, "Instructor is unable to edit writeboard data for non gradable multiple choice type question.");

        }
        catch (Exception e)
        {
            Assert.fail("Exception in testcase editWriteBoardResponseBeforeSubmittingGradableAssignment in class EditWriteBoardResponseBeforeSubmittingGradableAssignment.", e);
        }
    }
}
