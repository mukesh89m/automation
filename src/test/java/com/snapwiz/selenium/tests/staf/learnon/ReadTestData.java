...
public  class ReadTestData {
	private static Logger logger = Logger.getLogger("com.snapwiz.selenium.tests.staf.learnon.ReadTestData");
	public static String [] readData(String className, String tcIndex) {
		...
		try
		{
			...
			File fXmlFile = new File(workingDir+"/src/test/java/com/snapwiz/selenium/tests/staf/learnon/testdata_"+classNameTrimmed+".xml");
	...
	public static String readDataByTagName(String className, String tagName, String tcIndex) {
		...
		try
		{
			...
			if(className.contains("com.snapwiz.selenium.tests.staf.learnon.apphelper"))
			{
				className = new Exception().getStackTrace()[3].getClassName();
				classNametag = className;
			}
			if(className.equals("com.snapwiz.selenium.tests.staf.learnon.apphelper.Assignment"))
			{
				className = new Exception().getStackTrace()[4].getClassName();
				classNametag = className;
			}

			className = className.substring(50);
			className = className.replaceAll("\\.", "_");
			String classNameTrimmed= "";
			...
			String workingDir = System.getProperty("user.dir");
			File fXmlFile = new File(workingDir+"/src/test/java/com/snapwiz/selenium/tests/staf/learnon/testdata/testdata_"+classNameTrimmed+".xml");
