...
public class NewButtonActions {
	...
	public void verifyNewButton()
	{
		try
		{
            String discussionButton = Driver.driver.findElement(By.cssSelector("a[class='ls-toc-btn ls-right-new-btn']")).getText();
            if(!discussionButton.contains("+ New Discussion"))
				Assert.fail("+ New Discussion button is not present discussion tab in e-Textbook.");
            new com.snapwiz.selenium.tests.staf.AppiumLearningspaces.apphelper.Navigator().navigateToTab("Fav");
            List<WebElement> noteButtonOptions = Driver.driver.findElements(By.cssSelector("a[class='ls-toc-btn ls-right-new-btn']"));
	...
	public void verifyAddDiscussionBox()
	{
            try
            {
                ...
                Driver.driver.findElement(By.cssSelector("a[class='ls-toc-btn ls-right-new-btn']")).click();//click on + New Discussion button
                //Verifying if add discussion box is opened or not
                int cnt;
                if(Config.browser.equals("chrome"))
                     cnt = Driver.driver.findElements(By.cssSelector("div[class='editdiscussion-text']")).size();
                else
                     cnt = Driver.driver.findElements(By.cssSelector("div[class='editdiscussion-text']")).size();
                if(cnt == 0)
	...
	public void addDiscussion()
	{
		...
		{	
			...
			int cnt = Driver.driver.findElements(By.cssSelector("div[class='editdiscussion-text']")).size();	
			if(cnt != 0)
	...
	public void cancelButtonPressAddNoteBox()
	{
		...
		{	
            ...
            if(Driver.driver.findElements(By.className("stream-content-posts-list")).size() != 0)//if earlier notes are present
            {
                WebElement nopost = Driver.driver.findElement(By.className("stream-content-posts-list"));
