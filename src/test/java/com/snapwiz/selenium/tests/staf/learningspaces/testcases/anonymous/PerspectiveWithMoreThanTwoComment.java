...
public class PerspectiveWithMoreThanTwoComment extends Driver
{
	@Test
	public void perspectivewithmorethantwocomment()
	{
		try
		{
			String chaptername=ReadTestData.readDataByTagName("tocdata", "chapterName", "1");//fetch chapter name
			new Widget().createChapterWidget(2400);//create widget
			driver.quit();
			startDriver("firefox");
			new LoginUsingLTI().ltiLogin("2400");//login a instructor
