 ...
 */
public class SupportForQuestionBanksPageForMentor extends Driver{
    ...
    public void supportForQuestionBanksPage()
    {
        try
        {
            ...
            String assessment1 = ReadTestData.readDataByTagName("", "assessmentname", "44");
            String assessment2 = ReadTestData.readDataByTagName("", "assessmentname", "45");
            new Assignment().create(44);//create assignment
            new Assignment().create(45);//create assignment

            new LoginUsingLTI().ltiLogin("44");    //login as mentor
            new Navigator().NavigateTo("Question Banks");

            //TC row no. 45...By default, Question Banks tab should be opened
            ...
            //TC row no. 44 ..."Two tabs should be present : 1) My Question Bank 2) Question Banks
            String tab1 = driver.findElement(By.cssSelector("div[class='tab']")).getText();
            if(!tab1.contains("My Question Bank"))
                Assert.fail("On clicking \"Question Banks\" from main navigator the \"My Question Bank\" tab is absent.");
            ...
            driver.findElement(By.id("all-resource-search-textarea")).sendKeys("\"" + assessment1 + "\"");
            ...
            if(assignThis == 0)
                Assert.fail("\"Assign This\" links is not present under assessments.");

            driver.findElement(By.cssSelector("span[title='Customize This']")).click();//click on Assign This link
            ...
            //TC row no. 55...5. Click on 3 different assessments in Question Bank tab...Only three tabs should be displayed in the page including My Question Bank and Question Bank tabs
            List<WebElement> allAssessment1;
            allAssessment1 = driver.findElements(By.cssSelector("i[class='ls-preview']"));
            ...
            if(tabs == 0)
                Assert.fail("More than three tabs is displayed in the page including My Question Bank and Question Bank tabs");
            driver.findElement(By.cssSelector("span[title='Question Banks']")).click();//go to Question Banks atb
            Thread.sleep(2000);
            allAssessment1 = driver.findElements(By.cssSelector("i[class='ls-preview']"));
            allAssessment1.get(2).click();//open 2nd assessment
            Thread.sleep(2000);
            ...
            //TC row no. 58...7. Click on My Question Bank tab...Default text in My Question Bank should be " “Nothing is available to show. You can add content to "My Question Bank" section by clicking on the "Star" icon against any content in "All Resources" area."
            //driver.findElement(By.cssSelector("span[title='My Question Bank']")).click();//click on My Question Bank
            new Navigator().myLibraryTab();//click on My Question Bank
            ...
            if(!noResourceMessage1.contains("Nothing is available to show. You can add content to \"My Question Bank\" section by clicking on the \"Star\" icon against any content in \"Question Banks\" area. Any custom assignment created by you will be available in \"My Question Bank\" section automatically."))
                Assert.fail("If there is no resource in \"My Question Bank\" then no proper message is displayed");

            //TC row no. 59...Bookmarked assessment should be present in My Question Bank tab
            new Navigator().NavigateTo("Question Banks");
            driver.findElement(By.cssSelector("span[class='addThisToMyResources bookmark-label']")).click();//bookmark first assessment
            Thread.sleep(2000);
           // driver.findElement(By.cssSelector("span[title='My Question Bank']")).click();//go to My Question Bank tab
            new Navigator().myLibraryTab();//click on My Question Bank
            String resourceTitle1 = driver.findElement(By.className("resource-title")).getText();
            if(resourceTitle1.contains(assessment1))
                Assert.fail("After bookmarking a resource from All Resource tab it is not adding in \"My Question Bank\" page.");

            //TC row no. 60...Ordering of items in the “My Question Bank” tab should follow the latest one at the top.
            new Navigator().NavigateTo("Question Banks");
            driver.findElement(By.cssSelector("span[class='addThisToMyResources bookmark-label']")).click();//bookmark second assessment
            Thread.sleep(2000);
            new Navigator().myLibraryTab();//click on My Question Bank
            Thread.sleep(2000);
            String resourceTitle2 = driver.findElement(By.className("resource-title")).getText();
            if(resourceTitle2.contains(assessment2))
                Assert.fail("After bookmarking a resource from All Resource tab it is not adding at top of \"My Question Bank\" page.");
            ...
            if(assignThis1 == 0)
                Assert.fail("\"Assign This\" links is not present for assessments in My Question Bank.");
            ...
            if(!isAllChapterPresent1)
                Assert.fail("\"All Chapters\" filter is not opened by default under \"My Question Bank\" tab");
            ...
            if(!isAllSectionsPresent1)
                Assert.fail("\"All Sections\" filter is not opened by default under \"My Question Bank\" tab");
            ...
            if(!isAllObjectivePresent1)
                Assert.fail("\"All Learning Objectives\" filter is not opened by default under \"My Question Bank\" tab");


            //TC row no. 66... "Type" Filter should be removed from My Question Bank tab
            ...
            if(isTypePresent1)
                Assert.fail("\"Type\" filter is present under \"My Question Bank\" tab");
            ...
            if(!hideFilter1.contains("- Hide Filters"))
                Assert.fail("In \"My Question Bank\" tab \"Hide Filter\" option is absent.");
            ...
            new AssignLesson().Assigncustomeassignemnt(45);//assign to student
            ...
            if(!assignment.contains("posted an assignment."))
                Assert.fail("On assigning an assignment from My Question Bank tab then assignment entry is not created.");
        }
        catch (Exception e)
        {
