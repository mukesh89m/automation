 ...
 */
public class CreatingAndEditingAssignmentPolicy extends Driver {

    @Test
    public void creatingAndEditingAssignmentPolicy()
    {
        try
        {
            String policyname = ReadTestData.readDataByTagName("", "policyname", "2");
            ...
            //TC row no. 2......3. Click on "+New Assignment Policy" button..New option "Allow Collaboration" should be present under 'Other Policy Configuration' at the bottom of the New policy tab
            ...
            //TC row no. 3  //By default , “Allow Collaboration” should be in “disabled” state
            ...
            //TC row no. 4  //4. Click on "?"icon for Allow Collaboration policy----Help text should display “Select Enable if you want students to discuss assignment questions after each attempt.”
            ...
            //TC row no. 5...//5. Click on Enable option for Allow collaboration option....Instructor should be able to select enable for Allow Collaboration option
            ((JavascriptExecutor) driver).executeScript("arguments[0].click();", allElements.get(0));//click to enable the social collaboration

            //TC row no. 6...//"6. Enter a name for the policy and click on Save Policy button and Go to Assignment Policies tab"..New Assignment Policy should be created successfully
            ...
            if(!name.contains(policyname))
                Assert.fail("New Assignment policy is not created successfully after enabling the \"Allow Collaboration\".");

            //TC row no. 7...//8. Click on Update Policy..."Allow Collaboration" should be present under 'Other Policy Configuration'
            ...
            if(!allowCollaboration1.contains("Allow Collaboration"))
                Assert.fail("\"Allow Collaboration\" text is not present in creating assignment policy page on clicking update policy.");

            //TC row no. 8...//8. Click on Update Policy..."Allow Collaboration" should be in enabled state
        ...
        {
            Assert.fail("Exception in test case creatingAndEditingAssignmentPolicy in class CreatingAndEditingAssignmentPolicy.", e);
        }
    }
