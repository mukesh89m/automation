 ...
 */
public class VerifyGradebookForAssignmentWithImmediateFeedback extends Driver{

    @Test
    public void verifyGradebookForAssignmentWithImmediateFeedback()
    {
        try
        {
            ...
            //TC row no. 34..."Gradebook should show the new assignment cell with grade in the format: student score / Total score"
            new LoginUsingLTI().ltiLogin("34");    //login as instructor
            ...
            if(!score.equals("1/1"))
                Assert.fail("Gradebook for Assignment with immediate feedback does not show the new assignment cell with grade in the format: student score / Total score.");

            //TC row no. 35...Overall Score in percentage should be displayed in the Overall Score column
            ...
            if(!overallScore.get(1).getText().equals("100%"))
                Assert.fail("Gradebook for Assignment with immediate feedback does not show the Overall Score in percentage in the Overall Score column");

        }
        catch (Exception e)
        {
            Assert.fail("Exception in test case verifyGradebookForAssignmentWithImmediateFeedback in class VerifyGradebookForAssignmentWithImmediateFeedback.", e);
        }
    }
