...
public class MentorShowSpecificStudentPerformance
{
	@Test(priority=1,enabled=true)
	public void mentorShowSpecificStudentPerformance()
	{
		try
		{
			Driver.startDriver();
			new LoginUsingLTI().ltiLogin("1118");//login as student
			...
			new LoginUsingLTI().ltiLogin("11181");//login as instructor
		...
		catch(Exception e)
		{
			...
			Assert.fail("Exception in class MentorShowSpecificStudentPerformance in testcase mentorShowSpecificStudentPerformance",e);
		}
	}
	
	@Test(priority=2,enabled=true)
	public void mentorShowStudentPerformanceinspecificchapter()
	{
		try
		{
			Driver.startDriver();
			new LoginUsingLTI().ltiLogin("1119");
			new Navigator().NavigateToInstructorReport();
			new PerformanceReport().performancereport();
		...
		{
			new Screenshot().captureScreenshotFromTestCase();
			Assert.fail("Exception in class MentorShowSpecificStudentPerformance in testcase mentorShowStudentPerformanceinspecificchapter",e);
		}
	}
	@AfterMethod
