...
/**
 * Created by priyanka on 10/5/2015.
 */
public class CourseSpecificConfigurationsForLanguagePaletteForLs extends Driver {
    ...
    public void fileBasedAssessment() {
        try {
            ...
            new SelectCourse().selectLsCourse();//select course LS
    ...
    public void authorCreatedAsessmentWhichHaveTextEntryQuestion() {
        try {
            ...
            new DirectLogin().CMSLogin();//login in cms
            new SelectCourse().selectLsCourse();//select a course
            Thread.sleep(2000);
    ...
    public void instructorAbleToUseThePaletteInCourseStream() {
        try {
            ...
            String act=frenchText.substring(0,24);
            Assert.assertEquals(act, "ÀàÂâÆæÇçÈèÉéÊêËëÎîÏïÔôŒœ", "All the saved characters is not added to the POST text box.");
            driver.switchTo().defaultContent();
            courseStreamPage.languages.get(0).click();//click on language palette
            courseStreamPage.languageOptions.get(2).click();//click on french
            courseStreamPage.langaugePalette_header.click();
