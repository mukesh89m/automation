...
public class HighLight {
	...
	public void clickAndValidateHighlight(String tcIndex)
	{
		...
		{		
			...
			if(!postedatext.getText().trim().equals(postedstring.trim()))
				Assert.fail("Text 'posted a note' not present in NoteBox opened with arrow pointing downwards after clicking the text highlighted after adding a note/discussion");
	...
	public void clickAndValidateDiscussion(String tcIndex)
	{
		...
		{		
			...
			System.out.println(postedatext.getText().trim());
			if(!postedatext.getText().trim().equals(postedstring.trim()))
				Assert.fail("Text 'posted a note' not present in NoteBox opened with arrow pointing downwards after clicking the text highlighted after adding a discussion");
			
			WebElement staricon = Driver.driver.findElement(By.cssSelector("i[class='ls-right-section-sprites ls--right-star-icon']"));
			if(!staricon.isDisplayed())
				Assert.fail("NoteBox with arrow pointing downwards opened after clicking the text highlighted after adding a discussion doesnot contain star icon after text 'posted a discussion'.");
	...
	public void clickAndEditDiscussion()
	{
		try
		{
			...
			String random = new RandomString().randomstring(5);
			new com.snapwiz.selenium.tests.staf.dummies.apphelper.KeysSend().sendKeyBoardKeys(random+" ");
