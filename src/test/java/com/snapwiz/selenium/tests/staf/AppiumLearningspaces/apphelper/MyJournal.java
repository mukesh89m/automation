...
public class MyJournal {
    ...
    public void deleteNoteFromMyJournal(String dataIndex)
    {
        try
        {
            Driver.driver.findElement(By.className("my-journal-trash-icon")).click();//click on delete icon
            new UIElement().waitAndFindElement(By.className("confirm-study-note-delete"));
            Driver.driver.findElement(By.className("confirm-study-note-delete")).click();//click on Yes link on pop-up
            Thread.sleep(2000);
        }
        catch (Exception e)
        {
