...
{
	private static Logger logger = Logger.getLogger("com.snapwiz.selenium.tests.staf.dummies.apphelper.PerfomancePageVerification");
	...
	public int countOfBar()
	{
		int barcount=0;
		try
		{
			String numberofgraphbar=Driver.driver.findElement(By.className("al-content-performance-question-bars")).getAttribute("innerHTML");
			int totallength=numberofgraphbar.length();
			String Stringafterreplacement=numberofgraphbar.replace("div", "");
			int stringlength=Stringafterreplacement.length();
			int restlenght=totallength-stringlength;
			barcount=restlenght/6;
		}
		catch(Exception e)
		{
			Assert.fail("Exception in apphelper countofbar",e);
		}
		return barcount;
