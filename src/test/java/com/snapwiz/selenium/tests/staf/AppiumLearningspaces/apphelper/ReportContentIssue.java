...
public class ReportContentIssue {
    ...
    public void reportContentIssue(String issueText) {
        try {
            (new WebDriverWait(Driver.driver, 300))
                    .until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("div[title='Report Content Errors']")));
            Driver.driver.findElement(By.cssSelector("div[title='Report Content Errors']")).click();//click on report content issue
            Thread.sleep(2000);
            String url = Driver.driver.getCurrentUrl();
            if (url.contains("lsStudentDashBoard")) {
                /*Driver.driver.findElement(By.cssSelector("div[class='add-content-error show-content-issues-dialog']")).click();//click on report content issue(again clicking because of those help pop-up)
                Thread.sleep(2000);*/
                ...
                Thread.sleep(3000);
                Driver.driver.findElement(By.id("send-content-issue")).click();//click on Yes link on pop-up
                Thread.sleep(3000);
            } else {
                Driver.driver.findElement(By.id("text-area-content-issue")).click();//click on report content issue text area
                Driver.driver.findElement(By.id("text-area-content-issue")).sendKeys(issueText);//report text
    ...
    public void clickOnResolveButton(int index) {
        try
        {
            List<WebElement> allResolveButton = Driver.driver.findElements(By.className("content-error-resolved-it-btn"));//click on Resolve button
    ...
    public void clickOnReportContentIssueIcon() {
        try
        {
            Driver.driver.findElement(By.cssSelector("div[title='Report Content Errors']")).click();//click on report content issue icon
            Thread.sleep(2000);
        }
        catch (Exception e)
        {
    ...
    public void goToContentErrorReportPage() {
        try
        {
            Driver.driver.findElement(By.id("deliver-course")).click();//click on Summary in CMS
            Thread.sleep(2000);
            Driver.driver.findElement(By.className("review-details")).click();//click on right arrow icon next to "Content issue reported"
            Thread.sleep(2000);
        }
        catch (Exception e)
        {
    ...
    public boolean isIssuePresentInContentErrorReportPage(String issueText) {
        ...
        try
        {
            String errorText = Driver.driver.findElement(By.className("reported-content-issue-cmnts")).getText();
    ...
    public void reportContentIssueForPassageBaseQuestion(String issueText) {
        try
        {
            (new WebDriverWait(Driver.driver, 30))
                    .until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("div[title='Report Content Errors']")));
            List<WebElement> allIcon = Driver.driver.findElements(By.cssSelector("div[title='Report Content Errors']"));//click on report content issue
            ((JavascriptExecutor) Driver.driver).executeScript("arguments[0].click();", allIcon.get(1));//click on icon
            Thread.sleep(10000);//wait for help pop-up to disappear
            List<WebElement> textarea = Driver.driver.findElements(By.id("text-area-content-issue"));
            textarea.get(1).click();
            textarea.get(1).sendKeys(issueText);//report text
            List<WebElement> sendButtons = Driver.driver.findElements(By.id("send-content-issue-btn"));
            sendButtons.get(1).click();//click on report content issue send button
            List<WebElement> yesLinks = Driver.driver.findElements(By.id("send-content-issue"));
