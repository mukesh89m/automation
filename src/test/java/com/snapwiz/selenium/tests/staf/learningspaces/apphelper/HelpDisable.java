...
public class HelpDisable {
    ...
    public void helpDisable(String userId) {
        try {
            DBConnect.Connect();
            ResultSet rst = DBConnect.st.executeQuery("select id from t_user where external_id like '"+userId+"'");
            String id="";
            while (rst.next())
            {
                id=rst.getString("id");
            ...
            DBCollection userContextCollection = DBConnect.mongoConnect("UserContext");
