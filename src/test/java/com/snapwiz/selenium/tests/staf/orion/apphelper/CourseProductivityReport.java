...
public class CourseProductivityReport {
		...
		public void clickOnDot(int chapterNumber)
		{
			try
			{
			List<WebElement> allDots = Driver.driver.findElements(By.cssSelector("g.highcharts-markers > path"));
			allDots.get(chapterNumber-1).click();
			Thread.sleep(2000);
			}
			catch(Exception e)
			{
				Assert.fail("Exception in apphelper clickOnDot in class CourseProductivityReport.",e);
			}
		}
