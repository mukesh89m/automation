...
public class Bookmark {
	...
	public boolean isBookmarked(String type)
	{
		...
		try
		{
			...
			if(bookmark.contains("-32px -33px"))
				...
				String bookmark = Driver.driver.findElement((By.cssSelector("i[class='ls-right-section-sprites ls--right-star-icon-resource']"))).getCssValue("background-position");
				if(bookmark.contains("-77px -3px"))
					result = false;
				else
					result = true;
			}
				...
				String bookmark = Driver.driver.findElement((By.xpath("//*[starts-with(@class, 'ls-discussion-widget-bkmarking ls-discussion-widget-')]"))).getCssValue("background-position");
			
				if(bookmark.equals("-3px -7px"))
					result = true;
				else 
					result = false;
			}
	...
	public void isPresent(String type)
	{
		try
		{
			if(type.equalsIgnoreCase("lesson"))
			{
			int bookmark = Driver.driver.findElements(By.cssSelector("a[title='Bookmark']")).size();
			if(bookmark == 0)
				{
			...
			{
			int bookmark = Driver.driver.findElements(By.cssSelector("div[title='Bookmark']")).size();
			if(bookmark == 0)
				{
				new Screenshot().captureScreenshotFromTestCase();
	...
	public void bookmark(String type)
	{
		try
		{
			if(type.equalsIgnoreCase("lesson"))
			{
				...
				//Driver.driver.findElement(By.cssSelector("a.lesson-bookmark-sprite-img.bookmark-lesson")).click();
				Thread.sleep(2000);
			}
			...
			else if(type.equalsIgnoreCase("widget"))
			{
				...
				((JavascriptExecutor) Driver.driver).executeScript("arguments[0].click();", Driver.driver.findElement(By.cssSelector("div[title='Bookmark']")));
