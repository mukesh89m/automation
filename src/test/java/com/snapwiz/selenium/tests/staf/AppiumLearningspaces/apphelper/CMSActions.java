...
public class CMSActions {
    ...
    public void navigateToQuestionNo(int questionNo){
        try{
            new WebDriverWait(Driver.driver,180).until(ExpectedConditions.visibilityOf(Driver.driver.findElement(By.xpath("//a[@title='Jump To Q#']"))));
            Driver.driver.findElement(By.xpath("//a[@title='Jump To Q#']")).click();
            //((JavascriptExecutor)Driver.driver).executeScript("arguments[0].click();",Driver.driver.findElement(By.xpath("//a[@title='Jump To Q#']")));
            Thread.sleep(1000);
            ((JavascriptExecutor) Driver.driver).executeScript("arguments[0].scrollIntoView(true);",Driver.driver.findElement(By.linkText(Integer.toString(questionNo))));
