 ...
 */
public class DiscussionWidgetAsGradableAssignment extends Driver{

    @Test
    public void discussionWidgetAsGradableAssignment()
    {
        try
        {
            new LoginUsingLTI().ltiLogin("36_1");    //create student1
            new LoginUsingLTI().ltiLogin("36");		//login a instructor
            new Navigator().NavigateTo("eTextbook");	//navigate to etextbook
            new TopicOpen().openLessonWithDiscussionWidget();
            new DiscussionWidget().assignDiscussionWidgetWithDefaultClassSection(36);
            new Navigator().NavigateTo("Gradebook");    //go to Gradebook

            //TC row no. 36...DW Assignment name should be displayed in Gradebook
            ...
            if(assignmenName == null)
                Assert.fail("DW Assignment name is not displayed in Gradebook");

            //TC row no. 37..."Grades Not Released" text should be displayed in DW Assignment column
            ...
            if(!status.equals("Grades Not Released"))
                Assert.fail("\"Grades Not Released\" text is not displayed in DW Assignment column");
            ...
            //TC row no. 38...Overall Score for each student should be updated under Overall Score column
            ...
            if(!overallScore.get(1).getText().contains("100%"))
                Assert.fail("Overall Score for student does not updated under Overall Score column after grade is released.");

            //TC row no. 39...Grade of each student should be updated for DW assignment in Gradebook
            String status1 = driver.findElement(By.cssSelector("div[class='student-assignment-score-cell']")).getText();
            if(!status1.equals("2/2"))
                Assert.fail("Grade of student is not updated for DW assignment in Gradebook after release of grade.");

        }
        catch (Exception e)
        {
            Assert.fail("Exception in test case discussionWidgetAsGradableAssignment in class DiscussionWidgetAsGradableAssignment.", e);
        }
    }
