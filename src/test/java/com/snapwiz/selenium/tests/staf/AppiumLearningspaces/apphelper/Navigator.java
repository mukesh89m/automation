...
public class Navigator extends  Driver
{
    ...
    public void openResourceFromResourceTab(int dataIndex)
    {
        try
        {
            ...
            String actualResources= resoursename+appendChar;
            WebElement element=Driver.driver.findElement(By.xpath("//a[text()='"+actualResources+"']"));
    ...
    public void openResourceFromResourceTabFromCMS(int dataIndex)
    {
        try
        {
            ...
            navigateToResourceTab();
            int resindex = 0;
            List<WebElement> allOpenLink = Driver.driver.findElements(By.cssSelector("div[class='ls-right-section-status ls-right-section-resource']"));
            ...
            ((JavascriptExecutor) Driver.driver).executeScript("arguments[0].click();", allArrowIcon.get(resindex));	//click on arrow icon
            //click on resource
            List<WebElement> allOpenLink1 = Driver.driver.findElements(By.cssSelector("span[class='ls-right-tab-hover-sprite folder-cycle-bg']"));
            ((JavascriptExecutor) Driver.driver).executeScript("arguments[0].click();", allOpenLink1.get(resindex));
            Driver.driver.findElement(By.cssSelector("div.ls-main_aside > div.tabs > div.tab.active")).click();
    ...
    public void openFirstResourceFromResourceTab(int dataIndex)
    {
        try
        {
            String resoursename = ReadTestData.readDataByTagName("", "resoursename", Integer.toString(dataIndex));
            navigateToResourceTab();
            new WebDriverWait(Driver.driver,400).until(ExpectedConditions.presenceOfElementLocated(By.className("ls-resource-show-assign-this-block")));
