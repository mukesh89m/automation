...
public void navigateToDiscussionTab(int zerobasedTabIndex)
{
	try
	{
	List<WebElement> widgettabs = Driver.driver.findElements(By.cssSelector("span[class='publisher-count discussion-widget-publisherIcons-bg discussion-widget-publisher-count-bg-holder']"));  //Finding all tabs
	widgettabs.get(zerobasedTabIndex).click(); //clicking on a particular tab as per the index passed
	Thread.sleep(3000);
	}
	catch(Exception e)
...
public void clickonlikeofperspectivecomment(int liketoclick)
{
	...
	public void createChapterImageWidget(int dataIndex)
	{
		try
		{
		String course = Config.course;
